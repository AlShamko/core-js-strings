...
function countVowels(str) {
  const array = ['a', 'e', 'i', 'o', 'u', 'y', 'A', 'E', 'I', 'O', 'U', 'Y'];
  let count = 0;
  str.split('').forEach((el) => {
    if (array.includes(el)) count += 1;
  });
  return count;
}
...
function findLongestWord(sentence) {
  const arr = sentence.split(/\W+/);
  let longWord = '';
  arr.forEach((el) => {
    if (el.length > longWord.length) longWord = el;
  });
  return longWord;
}
...
function extractNameFromTemplate(value) {
  const cutHello = value.replace('Hello, ', '');
  return cutHello.replace('!', '');
}
...
function encodeToRot13(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    const strNumberCode = input.indexOf(str[i]);
    encode += output[strNumberCode];
  }
  return encode;
}
